    ...
    for line in lines[:30]:  # 检查前30行
        ...
        if line.startswith("#") and ("员工" in line or "employee" in line_lower):
            if "前端开发工程师" in line or ("前端" in line and "工程师" in line):
                ...
                "测试/运维工程师" in line
                or ("测试" in line and "工程师" in line)
                or ("运维" in line and "工程师" in line)
            ):
                return "测试/运维工程师"
            elif "产品经理" in line:
                return "产品经理"
    ...
    for i, line in enumerate(lines[:50]):  # 检查前50行
        if "register_agent" in line.lower() or "description" in line.lower():
            ...
            for j in range(i, min(i + 10, len(lines))):
                desc_line = lines[j]
                if "description" in desc_line.lower() and (
                    '"' in desc_line or "'" in desc_line
                ):
                    # 提取引号中的内容
...
def get_current_session_id() -> Optional[str]:
    ...
    for session_id, session_info in sessions.items():
        if session_info.get("agent_name") == agent_name and session_info.get(
            "active", False
        ):
            _current_session_id = session_id
            return session_id
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "topic": {"type": "string", "description": "需要帮助的主题"},
                    "description": {
                        "type": "string",
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "task_title": {"type": "string", "description": "完成的任务标题"},
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name == "send_message":
        ...
        if file_path:
            try:
                ...
                if file_path_obj.exists():
                    ...
                    return [
                        TextContent(type="text", text=f"错误: 文件不存在: {file_path}")
                    ]
        ...
        for msg in filtered_messages:
            read_status = (
                "✅ 已读"
                if msg.get("read", {}).get(current_agent, False)
                else "📩 未读"
            )
        ...
        return [
            TextContent(type="text", text=f"✅ 已标记 {updated_count} 条消息为已读")
        ]
        ...
        if not agent_name:
            return [TextContent(type="text", text="错误: 必须提供代理名称")]
        ...
        if not mdc_path.exists():
            return [TextContent(type="text", text=f"错误: .mdc文件不存在: {mdc_path}")]
        ...
        if auto_load_from_mdc:
            ...
            if agent_name in config:
                ...
                if mdc_content:
                    ...
                    if not description:
                        description = (
                            extract_description_from_mdc(mdc_content)
        ...
        if agent_tasks:
            ...
            for task in agent_tasks[:5]:  # 最多显示5个
                result_lines.append(
                    f"  - {task.get('id', '未知')}: {task.get('title', '未知')} ({task.get('status', '未知')})"
                )
        ...
        if not agents:
            return [TextContent(type="text", text="📋 没有已注册的AI代理")]
        ...
        for name, info in agents.items():
            ...
            session_info = sessions.get(session_id, {})
            active_status = (
                "🟢 活跃" if session_info.get("active", False) else "⚪ 非活跃"
            )
        ...
        task_title = (
            assigned_task.get("title", "未知任务") if assigned_task else "未知任务"
        )
        ...
        for task in tasks:
            ...
            if (
                assignee_filter
                and assignee_filter != "*"
                and task.get("assignee") != assignee_filter
            ):
                continue
        ...
        if not filtered_tasks:
            return [TextContent(type="text", text="📋 没有找到符合条件的任务")]
        ...
        if not recipients or not task_title or not summary:
            return [
                TextContent(type="text", text="错误: 必须提供接收者、任务标题和总结")
            ]
        ...
        if not recipients or not file_path or not description:
            return [
                TextContent(type="text", text="错误: 必须提供接收者、文件路径和描述")
            ]
        ...
        if not name or not members:
            return [TextContent(type="text", text="错误: 必须提供群组名称和成员列表")]
        ...
        if not group_id or not message:
            return [TextContent(type="text", text="错误: 必须提供群组ID和消息内容")]

        groups = load_groups()
        group = groups.get(group_id)

        if not group:
            return [TextContent(type="text", text=f"错误: 找不到群组 {group_id}")]

        if not group.get("active", True):
            return [TextContent(type="text", text=f"错误: 群组 {group_id} 已停用")]

        current_agent = get_current_agent()
        if current_agent not in group.get("members", []):
            return [
                TextContent(type="text", text=f"错误: 你不是群组 {group_id} 的成员")
            ]
        ...
        if reply_to:
            ...
            if reply_msg:
                reply_info = {
                    ...
                    "reply_to_content": reply_msg.get("content", "")[
                        :200
                    ],  # 只保存前200字符
                }
        ...
        if not group_id:
            return [TextContent(type="text", text="错误: 必须提供群组ID")]
        ...
        # 格式化消息
        result_lines = [
            f"📬 群组消息 ({group.get('name', group_id)}): 找到 {len(filtered_messages)} 条\n"
        ]
        for msg in filtered_messages:
            read_status = (
                "✅ 已读"
        ...
        if not groups:
            return [TextContent(type="text", text="📋 没有群组")]
        ...
        if not filtered_groups:
            return [TextContent(type="text", text=f"📋 没有找到符合条件的群组")]
        ...
        for group_id, group_info in filtered_groups:
            ...
            if group_status == "active":
                ...
                result_lines.append(
                    f"状态: {'🟢 活跃' if group_info.get('active', True) else '⚪ 停用'}"
                )
            ...
            if include_preview:
                ...
                if group_messages:
                    ...
                    result_lines.append(
                        f"   时间: {last_msg.get('timestamp', '')[:19]}"
                    )
        ...
        for msg in group_messages:
            ...
            if any(kw in content for kw in ["决定", "决策", "确定", "采用"]):
                decisions.append(
                    f"- {sender} ({role}): {msg.get('content', '')[:100]}..."
                )
        ...
        if not query_groups:
            query_groups = [
                ...
                if current_agent in g.get("members", [])
        ...
        if total_unread == 0:
            ...
            result_lines.append(
                f"📈 总计: {total_unread}条未读 | {total_mentions}条@我 | {total_important}条重要"
            )
        ...
        if current_agent != creator:
            return [
                TextContent(
                    type="text", text=f"错误: 只有创建者（{creator}）可以归档群组"
                )
            ]
        ...
        if not group_id or not message_id:
            return [TextContent(type="text", text="错误: 必须提供群组ID和消息ID")]
        ...
        if not message or message.get("group_id") != group_id:
            return [TextContent(type="text", text=f"错误: 找不到消息 {message_id}")]
        ...
        if check_tasks:
            ...
            agent_tasks = [
                t
                for t in tasks
                if t.get("assignee") == current_agent
                and t.get("status") in ["待开始", "进行中"]
            ]
            if agent_tasks:
        ...
        if has_new_items:
            ...
            if found_messages:
                ...
                for msg in found_messages[:3]:  # 最多显示3条
                    result_lines.append(f"\n--- 消息 {msg.get('id', '未知')} ---")
                    result_lines.append(
                        f"发送者: {msg.get('sender', '未知')} ({msg.get('sender_role', '未知角色')})"
                    )
            ...
            result_lines.append(
                "💡 请处理上述任务/消息，处理完成后立即调用standby继续监听"
            )
            result_lines.append(
                "📋 操作：处理完成后，在回复末尾调用standby({{'status_message': '任务处理完成，继续监听'}})"
            )
            ...
            if remaining_seconds > 0:
                ...
                result_lines.append(
                    f"💡 提示: 暂无新任务/消息，但必须继续调用standby才能保持监听状态"
                )
                result_lines.append(
                    f"📋 操作：请在回复末尾继续调用standby，直到收到新任务/消息或5分钟到期"
                )
                result_lines.append(
