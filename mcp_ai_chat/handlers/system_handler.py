...
def extract_description_from_mdc(mdc_content: str) -> str:
    ...
    match = re.search(
        r"description\s*[：:]\s*(.+)", mdc_content, re.IGNORECASE | re.DOTALL
    )
    if match:
        return match.group(1).strip()
    return ""
...
async def handle_register_agent(arguments: dict[str, Any]) -> list[TextContent]:
    ...
    agent_tasks = [
        t
        for t in tasks
        if t.get("assignee") == agent_name and t.get("status") in ["待开始", "进行中"]
    ]
    ...
    if agent_tasks:
        ...
        for task in agent_tasks[:3]:  # 最多显示3个
            result_lines.append(
                f"  - {task.get('title', '未知')} (优先级: {task.get('priority', '未知')})"
            )
...
async def handle_list_agents(arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if not agents:
        return [TextContent(type="text", text="📋 当前没有已注册的AI代理")]
